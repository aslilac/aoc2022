...
fn partially_overlapped(a: Range, b: Range) -> Bool {
  a.begins_at <= b.ends_at && a.begins_at >= b.begins_at || b.begins_at <= a.ends_at && b.begins_at >= a.begins_at
}
...
pub fn main() {
  ...
  |> list.filter(fn(pair) { partially_overlapped(pair.0, pair.1) })
  |> list.length()
  |> io.debug
}
